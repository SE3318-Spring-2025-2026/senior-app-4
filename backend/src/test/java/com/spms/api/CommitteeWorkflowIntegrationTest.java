...
public class CommitteeWorkflowIntegrationTest {
    ...
    void scenario1_CompleteAssignmentWorkflow() {
        assertTimeout(Duration.ofSeconds(2), () -> {
            ...
            String committeeJson = "{\"committeeName\": \"Senior Project Comm\", \"status\": \"ACTIVE\"}";
            ...
            Long committeeId = JsonPath.read(result.getResponse().getContentAsString(), "$.committeeId");
            ...
            mockMvc.perform(get("/api/v1/committees/" + committeeId)
                    ...
                    .andExpect(jsonPath("$.advisors[0].advisorId").value(200))
                    .andExpect(jsonPath("$.groupAssignments[0].groupId").value(50));
    ...
    void scenario2_JuryLifecycleAndAudit() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/v1/committees")
                .requestAttr("jwt_userId", 100L)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"committeeName\": \"Jury Comm\", \"status\": \"ACTIVE\"}"))
                .andExpect(status().isCreated())
                .andReturn();
        Long committeeId = JsonPath.read(result.getResponse().getContentAsString(), "$.committeeId");
    ...
    void scenario3_ScheduleConflict_Returns400() throws Exception {
        ...
        MvcResult result = mockMvc.perform(post("/api/v1/committees")
                .requestAttr("jwt_userId", 100L)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"committeeName\": \"Conflict Comm\", \"status\": \"ACTIVE\"}"))
                .andExpect(status().isCreated())
                .andReturn();
        Long committeeId = JsonPath.read(result.getResponse().getContentAsString(), "$.committeeId");
    ...
    void scenario4_FilterUpdateDeleteLifecycle() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/committees")
                .requestAttr("jwt_userId", 100L)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"committeeName\": \"List Comm\", \"status\": \"FORMING\"}"))
                .andExpect(status().isCreated());
        ...
        Long committeeId = JsonPath.read(getResult.getResponse().getContentAsString(), "$.data[0].committeeId");
        
        // 3. Update Committee (PUT)
        mockMvc.perform(put("/api/v1/committees/" + committeeId)
                .requestAttr("jwt_userId", 100L)
                .contentType(MediaType.APPLICATION_JSON)
                .content("{\"committeeName\": \"Updated Comm\", \"status\": \"ACTIVE\"}"))
                .andExpect(status().isOk());
