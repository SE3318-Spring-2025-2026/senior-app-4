...
class AdvisorRequestDecisionServiceTest {
    ...
    void approveSetsAdvisorAndIncrementsWorkload() {
        ...
        when(notificationRepository.findByGroupIdAndTypeAndStatusAndToUser_UserIdNot(any(), any(), any(), any()))
                .thenReturn(Collections.emptyList());

        AdvisorDecisionResponseDto response = groupService.processAdvisorRequestDecision(10L, 1L, "APPROVE", null);
        ...
        assertEquals(ActionType.ADVISOR_ASSIGNED, auditLogCaptor.getValue().getActionType());
    }
    ...
    void approveAutoRejectsOtherPendingRequests() {
        ...
        when(groupRepository.findById(100L)).thenReturn(Optional.of(group));
        when(userRepository.findById(10L)).thenReturn(Optional.of(professor));
        when(notificationRepository.findByGroupIdAndTypeAndStatusAndToUser_UserIdNot(100L, NotificationType.ADVISOR_REQUEST, NotificationStatus.PENDING, 10L))
    ...
    void rejectUpdatesStatusAndRecordsReason() {
        ...
        AdvisorDecisionResponseDto response = groupService.processAdvisorRequestDecision(10L, 1L, "REJECT", "No capacity");
        ...
        ArgumentCaptor<AuditLog> auditLogCaptor = ArgumentCaptor.forClass(AuditLog.class);
        verify(auditLogRepository).save(auditLogCaptor.capture());
        assertEquals(ActionType.ADVISOR_REJECTED, auditLogCaptor.getValue().getActionType());
