...
public class SubmissionGradeService {
        ...
        @Transactional
        public GradeSubmissionResponse submitGrade(Long submissionId, Long professorId,
                ...
                if (gradeRepository.existsBySubmissionIdAndProfessorId(submissionId, professorId)) {
                        throw new IllegalArgumentException(
                                        "Professor has already submitted a grade for this submission");
                }
                ...
                GroupCommitteeAssignment assignment = assignmentRepository
                                ...
                                .orElseThrow(() -> new IllegalStateException(
                                                "No active committee assigned to this group"));
                ...
                if (allGrades.size() == totalCommitteeMembers) {
                        ...
                        // NotificationService.
                        String notificationMsg = "Grading is complete for Submission " + submissionId
                                        + ". Final grade: "
                                        + String.format("%.2f", average);

                        // Notify Coordinator (FIX-3)
                        notificationService.createSystemAlert(committee.getCreatedBy(), notificationMsg,
                                        "GRADING_COMPLETE",
                        ...
                        Group group = groupRepository.findById(submission.getGroupId())
                                        .orElseThrow(() -> new IllegalStateException(
                                                        "Group not found for submission: " + submission.getGroupId()));

                        if (group.getLeader() != null) {
                                notificationService.createSystemAlert(group.getLeader().getUserId(), notificationMsg,
                                                "GRADING_COMPLETE",
                                                "{\"submissionId\": " + submissionId + "}");
                        }
                }

                return new GradeSubmissionResponse("success", "Grade submitted successfully.", grade.getId(),
                                isGradingComplete);
        }
}
