...
type ContentMode = "file" | "markdown";
...
export default function StudentSubmissionForm({
  ...
  const handleSubmit = async () => {
    ...
    if (contentMode === "markdown" && !markdownContent.trim()) {
      toast.error("The document cannot be empty.");
      return;
    }
    ...
    try {
      if (isRevisionMode) {
        const payload = await createRevisionSubmission(parentSubmissionId!, {
          file: contentMode === "file" && file ? file : undefined,
          description: revisionDescription,
          content: contentMode === "markdown" ? markdownContent : undefined,
        });
        setStatus("success");
        ...
        <div className="border-b border-white/5 px-6 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              {isRevisionMode
                ? "Review comments first, then upload the revised document"
                : "Select a deliverable type and provide your content"}
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-6 p-6 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-5">
            ...
            {!isRevisionMode && (
              <div className="space-y-2">
                ...
                <div className="rounded-xl border border-white/10 bg-white/5">
                  ...
                  >
                    <option value="" className="bg-gray-950 text-gray-400">Select Proposal or SoW</option>
                ...
                <p className="text-xs text-gray-500">
                  {deliverableType
                    ? deliverableOptions.find((o) => o.value === deliverableType)?.hint
            ...
            <div className="space-y-3">
              ...
              {contentMode === "file" && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-gray-500">Accepted: PDF, DOC, DOCX — max 20 MB</span>
                  </div>
                  <div
                    onDragOver={handleDragOver}
                    onDragLeave={handleDragLeave}
                    onDrop={handleDrop}
                    onClick={() => { if (!disabled && status !== "uploading") fileInputRef.current?.click(); }}
                    className={[
              ...
              {contentMode === "markdown" && (
                <MarkdownEditor
                  ...
                  disabled={disabled || status === "uploading"}
                  placeholder="Start writing your document here. Use the toolbar for formatting, or drag & drop images directly into the editor."
                />
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {(file || deliverableType || revisionDescription || markdownContent) && status !== "uploading" && (
                <button
                  type="button"
                  onClick={handleReset}
            ...
            <div className="bg-gray-950/40 border border-white/8 rounded-2xl overflow-hidden">
              <div className="px-5 py-4 border-b border-white/5">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {isRevisionMode
                    ? "This flow links the new content to the requested revision."
                    : "Provide your deliverable as a file upload or write it directly in the editor."}
                </p>
              </div>
              <div className="p-5 space-y-4 text-sm text-gray-300">
                ...
                <div className="rounded-xl border border-white/8 bg-white/4 px-4 py-3">
                  <p className="text-xs text-gray-500">Content modes</p>
                  <p className="mt-1 text-white">File Upload (PDF/DOC/DOCX) or Markdown Editor</p>
                </div>
                <div className="rounded-xl border border-white/8 bg-white/4 px-4 py-3">
                  <p className="text-xs text-gray-500">Submission target</p>
                  <p className="mt-1 font-mono text-blue-200">
                    {isRevisionMode
                      ? `POST /submissions/${parentSubmissionId ?? "{id}"}/revisions`
                      : "POST /submissions"}
                  </p>
                </div>
              </div>
              ...
              <div className="p-5 space-y-3 text-sm text-gray-300">
                <div className="flex items-start gap-3">
                  <span className={`mt-1 h-2.5 w-2.5 rounded-full ${
                    isRevisionMode
                      ? parentSubmissionId ? "bg-green-400" : "bg-gray-600"
                      : deliverableType ? "bg-green-400" : "bg-gray-600"
                  }`} />
                  <span>{isRevisionMode ? "Parent submission is linked." : "Deliverable type is selected."}</span>
                </div>
                <div className="flex items-start gap-3">
                  <span className={`mt-1 h-2.5 w-2.5 rounded-full ${
                    (contentMode === "file" && file) || (contentMode === "markdown" && markdownContent.trim())
                      ? "bg-green-400"
                      : "bg-gray-600"
                  }`} />
                  <span>
                    {contentMode === "file" ? "A supported file is attached." : "Markdown content is provided."}
                  </span>
                </div>
                <div className="flex items-start gap-3">
              ...
              <div className="bg-gray-900 border border-green-500/20 rounded-2xl overflow-hidden">
                ...
                <div className="p-5 space-y-2 text-sm text-gray-300">
                  ...
                  {lastSubmission.submittedAt && (
                    <p>Submitted at: <span className="text-white">{new Date(lastSubmission.submittedAt).toLocaleString("en-US")}</span></p>
                  )}
                </div>
              </div>
            )}
