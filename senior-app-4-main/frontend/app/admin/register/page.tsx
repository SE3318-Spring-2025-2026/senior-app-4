...
export default function AdminProfessorRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. Client-side validation: Strong Password Check
    ...
    try {
      // 3. Backend Endpoint Integration (Fixed absolute URL for CORS)
      const response = await fetch("http://localhost:8080/api/v1/professors/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
      });

      // API bazen text bazen json dönebilir, hata kontrolü ekliyoruz
      let data;
